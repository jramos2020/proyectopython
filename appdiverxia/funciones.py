...
def productoTieneIgv(idProducto):
    igv = Producto.objects.get(id=idProducto)
    resultado = igv.tiene_igv
    
    return resultado

def sacarIgv(elemento):
    igv = Opciones.objects.get(id=1)
    igvSacado =  igv.valor_igv/100
    resultado = elemento + (elemento * Decimal(igvSacado))
    return resultado    

def igvActual(modo):
    if modo == 'valor':
        igv = Opciones.objects.get(id=1)
        return igv.valor_igv
